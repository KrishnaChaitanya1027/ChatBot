...
const port = process.env.PORT || parseInt(process.argv.pop()) || 3004;

server.listen(port, function () {
  console.log("Server listening at port %d", port);
});
const DosaOrder = require("./assignment1Dosa");
...
app.post("/payment", (req, res) => {
  ...
  oOrders[sFrom] = new DosaOrder(sFrom);
...
app.post("/sms", (req, res) => {
    ...
    if (!oOrders.hasOwnProperty(sFrom)) {
      oOrders[sFrom] = new DosaOrder(sFrom, sUrl);
    }
    if (oOrders[sFrom].isDone()) {
      delete oOrders[sFrom];
    }
...
// var port = process.env.PORT || parseInt(process.argv.pop()) || 3004;
